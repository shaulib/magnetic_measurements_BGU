...
spt = sp*8;
gradient=1;
sgrad1=zeros(spt,1);sgrad2=sgrad1;
% auto file detecter:
...
% title(filetitle);
% 
% title(strcat(filetitle,'_',' ch1 and ch3'));
% xlabel('Hz');
...
index = strfind(filename,'_');
filetitle = char(extractBetween(filename,1,index(2)-2));
final_index = strfind(filename,'.');
filetitle = strcat(filetitle, num2str(expNum-1),char(extractBetween(filename,(index(2)),final_index(1)-1)));
filetitle = char(extractBetween(filetitle,1,final_index(1)-2));
filetitle = strcat(filetitle, num2str(Gnum),'_');
Gnum = Gnum + 1;
% title(strcat(filetitle, '_Gradient'));
title(filetitle);
xlabel('Hz');
